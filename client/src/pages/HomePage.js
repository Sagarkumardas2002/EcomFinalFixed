...
const HomePage = () => {
    ...
    const getAllCategory = async () => {
        try {
            ...
            if (data?.success) {
                setCategories(data?.category);
                setVisibleCategories(data?.category.slice(0, 8));
            }
        } catch (error) {
            console.log(error);
        }
        ...
        if (categoriesRef.current) {
            ...
            if (scrollTop + clientHeight >= scrollHeight - 5) {
                setVisibleCategories(prevVisibleCategories => [
                    ...
                    ...categories.slice(prevVisibleCategories.length, prevVisibleCategories.length + 8)
                ]);
            }
        }
    };
        ...
        <Layout title={"Sagar's Ecom App-Shop Now....."}>
            <div className="container-fluid row mx-auto">
                <div className="col-md-2 col-sm-12">
                    <h4 className="text-center mt-4" style={{ color: "maroon" }}>Filter By Category</h4>
                    <hr />
                    <div className="d-flex flex-column" ref={categoriesRef} onScroll={handleScroll} style={{ maxHeight: '200px', overflowY: 'auto' }}>
